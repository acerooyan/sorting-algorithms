...
std::vector<int> merge_nums3(std::vector<int> l, std::vector<int> r)
{
...
void hybrid_sort3_helper(std::vector<int>& nums, int H){
    ...
    if (nums.size() > H)
    {
        ...
        hybrid_sort3_helper(left,H);
        hybrid_sort3_helper(right,H);
        nums = merge_nums3(left, right);
...
void hybrid_sort3(std::vector<int>& nums){
    ...
    int H = pow(nums.size(), 1/ double(6));

    hybrid_sort3_helper(nums, H);
